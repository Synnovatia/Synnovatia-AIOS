...
| Missing trailing slash | 2 | `/monthly-coaching` (no trailing `/`) | The real page exists at `/monthly-coaching/` (with slash) — something is linking to it without the trailing slash, and the site isn't auto-redirecting no-slash → slash for this path. Cheapest fix on this whole list. |
...
3. Fix the two missing-trailing-slash links directly (cheapest win, no lookup needed)
