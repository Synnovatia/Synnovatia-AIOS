...
│   └── website-redesign/     # "Different Is Better Than Better" website homepage/About/Work With Me/Messy Middle copy + mockups. 2026-08-04-homepage-merged.html, 2026-08-04-about-page-mockup.html, and 2026-08-12-work-with-me-mockup.html are canonical and deployed live on the DreamHost staging site (synnovatiacom.stage.site — password-protected, separate from WP login, not publicly indexed): homepage at /homepage-new-design-draft/, About at /about/ (replaced old 2020 content directly), Work With Me at /work-with-me/ (new page on a clean slug, deployed 2026-08-13). **Nav architecture updated 2026-08-13:** About / The Messy Middle / Masterminds / Work With Me / Blog — "Masterminds" is a new planned page (not yet built) combining Mastermind for the Messy Middle + Seven Figure Forum, replacing the old plan where Seven Figure Forum had no home anywhere on the site. 2026-08-13-messy-middle-core-mockup.html is the built-but-not-deployed concept/positioning page for "The Messy Middle" nav item (adapted from a richer reference file Jackie had built outside the workspace), ending in pathway cards to the future Masterminds page and to Work With Me; a real predecessor page already exists at the old site's /the-messy-middle/, so deploying this will need a decision (overwrite vs. new page) that Jackie hasn't made yet. 2026-08-13-messy-middle-mockup.html (concept + Mastermind-for-the-Messy-Middle program details/pricing/apply-CTA on one page) is superseded by that nav change — its program-details content is the natural starting point for the future Masterminds page once that gets built. Client quote sign-off status for all live pages tracked in the `project_website-redesign-status` memory, not here. Deploy = paste into the page's Custom HTML block in the block editor, PLUS append two CSS blocks not present in the source mockup file: a WordPress theme override (`#masthead, #site-navigation, div.site-footer, footer.site-info, .entry-header { display: none !important; }` + full-bleed width overrides + `.widget-area.sidebar { display: none !important; }`) to suppress the GeneratePress theme's own header/nav/title/sidebar, and a button-visibility fix (`!important` on button text/background) since the theme's link-color CSS otherwise washes out the mockup's teal buttons — verify both via `getComputedStyle` on `.site-header`, not just the "Edit code" preview. Also swap any `src="assets/..."` logo path for the real WP media-library URL. Then verify via "Edit code" preview before Update, then Save — WP admin has no reliable way to detect a silent failed save, so always re-verify via the REST API (`/wp-json/wp/v2/pages/<id>`) and a cache purge ("Purge Current Page" in the admin bar) after saving, not just the visual "Saved" state. GeneratePress child theme CSS (`h1-h6`, `.entry-content h2/h3/h4`, `p + p`) silently overrides same-or-lower-specificity page CSS — any pasted color/spacing rule needs `!important` to hold
