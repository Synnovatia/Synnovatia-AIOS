...
## Email Visual Styling Recipe (confirmed 2026-08-27)

Every HubSpot marketing email draft Claude creates should now carry this styling, applied via `manage_marketing_email` EDIT_CONTENT rather than left at HubSpot's unstyled defaults. Grounded in `context/style-guide.md`'s actual brand tokens, not invented — this replaces the earlier practice of shipping drafts with no explicit font/color/layout set (which meant HubSpot's own default blue link color and system font rendered instead of the brand).

- **Body text:** `baseFontStyle` — font family `"Barlow, Arial, sans-serif"` (HubSpot serves Barlow as a real webfont via Google Fonts, Arial is the documented print/email fallback per style-guide.md), size 16px, color `#444441` (Body Text token). Also set inline on every `<p>` in EmailTextModule HTML for reliability across email clients that don't honor cascaded base styles, plus `line-height:1.7` (matches the Body type-scale row's "generous" spacing note).
- **Paragraph spacing gotcha (found 2026-08-27, fix required):** HubSpot's own base email CSS sets `p{margin:0}` on every marketing email, confirmed via `PREVIEW_CONTENT`'s rendered HTML — so multiple `<p>` tags inside one `EmailTextModule` render back-to-back with zero visual gap between them, `line-height` alone does nothing to separate paragraphs. The first version of this recipe missed this and shipped a draft that read as a cramped wall of text; Jackie caught it and worked around it by hand-inserting empty `<p>&nbsp;</p>` spacer paragraphs between every real paragraph. **Do this instead going forward:** add explicit `margin-bottom` (e.g. `16px`) inline on every `<p>` — `style="font-family:Barlow, Arial, sans-serif; font-size:16px; line-height:1.7; color:#444441; margin:0 0 16px 0;"` — so paragraphs space themselves correctly without needing manual spacer paragraphs.
- **Links:** `baseLinkFontStyle` and inline `<a>` styles — color `#0F6E56` (Teal, "trust/empathy" per the palette notes), underlined. Applies to inline text links and the footer's unsubscribe/manage-preferences links.
- **CTA buttons (EmailButtonModule):** background `#0F6E56` (Teal), white `#FFFFFF` bold text, 10px corner radius (matches the site's real `.btn-primary` radius), font `"Barlow Condensed, Arial, sans-serif"` at 15px (Barlow Condensed is the brand's Labels/UI-Detail typeface). Left-aligned, not centered — a big centered marketing-blast button reads more "guru" than "peer"; left-aligned under body copy keeps the personal-email feel.
- **Section padding:** section-0 (the content section) gets `paddingTop: 32px` / `paddingBottom: 16px` desktop, `24px`/`12px` mobile, instead of the previous 0px/0px — gives the email some breathing room without feeling padded. Section-1 (footer) stays at 0px, matching the footer module's own built-in spacing.
- Verified via `PREVIEW_CONTENT` (renderedHtml) that HubSpot actually serves Barlow/Barlow Condensed as `@font-face` webfonts (`hs-webfonts`), so this isn't just wishful CSS — it renders as intended in clients that support webfonts, with Outlook/MSO falling back to Arial per HubSpot's own conditional CSS block.
- **Editor quirk to watch for, not a Claude mistake:** after Jackie hand-edits a draft in HubSpot's own rich-text editor, `GET_CONTENT`'s `baseFontStyle.fontFamily` (the document-level base style, distinct from each paragraph's own inline style) has been seen to silently change to an unrelated CJK font stack (`'PingFang TC', 微軟正黑體, 'Microsoft JhengHei', Arial, sans-serif` — found 2026-08-27) with no corresponding action from her side. Each paragraph's inline `font-family` stays correct, so this likely has no visible rendering effect, but check `baseFontStyle` after any live hand-edit before assuming the recipe is still intact, and don't silently "fix" it mid-edit — flag it to Jackie since it may indicate a browser/locale quirk worth her knowing about.

**Why:** Jackie asked (2026-08-27) to update font size, color, and layout on the live HubSpot drafts and make it part of the permanent record, after drafts had been shipping with HubSpot's unstyled defaults rather than actual brand styling. Same underlying instinct as [[feedback_no-ai-design-patterns]] — every visual choice should trace to a documented brand token, not a platform default or an invented one.

**How to apply:** Every future `manage_marketing_email` CREATE + EDIT_CONTENT pass (Active Engagers, Drifting, Lapsed, standalone sends) should default to this recipe rather than plain unstyled modules. If a specific send calls for a deliberate deviation, note why in that draft's file rather than silently reverting to defaults.
...
- **Messy Middle-fit women (full 397 segment) list ID found 2026-08-25:** **407** (exclude 377, 219, 383) — pulled from the real "Email: Women Only Mastermind (07/16/26)" send via `GET_EMAIL_DETAILS`. Use this for any standalone send to the full Messy Middle-fit segment, distinct from list 404 (the smaller Active-Engagers-overlap subset).
