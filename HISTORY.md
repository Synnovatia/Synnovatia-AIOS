...
- **Email suppression only partly succeeded**, and the gap is documented: `hs_email_optout` is read-only via the API; `hs_marketable_status` is blocked because the connector lacks the `marketable-contacts-write` scope. Only `portal_unsubscribe` could be set (verified true), and its enforcement at send time couldn't be confirmed from here. Two manual HubSpot steps left for Jackie: verify the unsubscribe on her record, and set her as a non-marketing contact. **Third connector limitation found today** — no list management, no marketable-contacts write, canonical opt-out read-only.
