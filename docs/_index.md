...
| Pre-meeting objective emails (existing clients) | `context/meeting-prep.md` | Boomerang booking → 4-day-out objective ask if not captured at booking, tracked in `data/meeting-prep/tracking.csv`, daily `pre-meeting-objective-check` scheduled task. Excludes new clients (handled by onboarding instead) |
