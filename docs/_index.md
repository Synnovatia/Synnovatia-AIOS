...
| Drafting HubSpot marketing emails (Active Engagers/Drifting/Lapsed segments) | `context/hubspot-marketing.md` | Segment definitions, cadence per segment, "What I'm Watching" content thread. Since 2026-08-06: Claude drafts copy AND creates it as a real draft in HubSpot (`hubspot-*-draft` Thursday tasks), `hubspot-send-stats-tracker` (Mondays) pulls real send analytics and logs a style diff (drafted vs. shipped) to `outputs/hubspot-marketing/style-learning-log.md`, and `hubspot-style-pattern-review` (monthly) surfaces recurring edit patterns for Jackie to confirm as standing rules. Sending itself has no exposed API at all — stays entirely Jackie's action in HubSpot |
...
| Drafting or sending general inbox email replies | `docs/email-response.md` | `email-reply-drafting-check` (every 2 hrs, 6am-6pm) drafts Jackie-voice replies on any inbox thread awaiting a response, as real Gmail drafts — review happens in Gmail itself. The one system where Claude can also execute a send, but only reactively when Jackie names a specific email in chat (via browser automation on her real Gmail, since the connector has no send API) — never on a schedule, never automatic |
