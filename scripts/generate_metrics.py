...
def section_stripe(conn):
    """Stripe — invoice-based revenue (Synnovatia bills via Invoices, not Subscriptions)."""
    if not table_exists(conn, "stripe_daily"):
        return []
    lines = []
    lines.append("## Revenue (Stripe)")
    lines.append("| Metric | Value | As Of |")
    lines.append("|--------|-------|-------|")
    rows = query_all(conn, """
        SELECT * FROM stripe_daily
        WHERE date = (SELECT MAX(date) FROM stripe_daily)
        ORDER BY account
    """)
    for r in rows:
        lines.append(f"| Revenue MTD ({r['account']}) | {fmt_currency(r['revenue_mtd'])} | {r['date']} |")
        lines.append(f"| Revenue YTD ({r['account']}) | {fmt_currency(r['revenue_ytd'])} | {r['date']} |")
        lines.append(f"| Refunds MTD | {fmt_currency(r['refunds_mtd'])} | {r['date']} |")
        lines.append(f"| Invoices paid MTD | {fmt_number(r['invoices_paid_mtd'])} | {r['date']} |")
        lines.append(f"| Customers billed MTD | {fmt_number(r['customers_billed_mtd'])} | {r['date']} |")
        lines.append(f"| Total customers (all-time) | {fmt_number(r['total_customers'])} | {r['date']} |")
    lines.append("")
    lines.append("_Note: Revenue = net Charges (gross charges minus refunds), matching the Stripe "
                  "dashboard total. Jackie's self-reported total income (including any non-Stripe "
                  "payments) is tracked separately in `context/current-data.md`._")
    lines.append("")
    return lines
